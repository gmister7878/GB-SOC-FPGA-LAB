...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    // 2048 reads of a 32-bit register => 2048 * 4 bytes
    unsigned int bytes_transferred = 2048 * sizeof(unsigned int);
    // Timer increments at 125 MHz (same clock used elsewhere in this design)
    float time_spent = (float)elapsed_clocks / 125.0e6f;
    float throughput = ((float)bytes_transferred / time_spent) / 1.0e6f; // MB/s (10^6)
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Gerald Barnett - Custom Peripheral Demonstration\n\r");
